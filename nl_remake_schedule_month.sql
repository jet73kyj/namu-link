-- ===========================================================
-- nl_remake_schedule_month(요일줄번호, 달)
--   그 요일줄 하나만 그 달 회기를 다시 깐다.
--   완료·결석·보강 회기는 건드리지 않는다.
--
--   2026-08-29 시점 원문 보관본
--   ※ 앞으로 고칠 때는 이 파일을 고쳐서 통째로 다시 만들 것.
--     글자를 찾아 바꾸는 방식(replace)은 쓰지 말 것.
-- ===========================================================
create or replace function public.nl_remake_schedule_month(p_schedule_id bigint, p_month text) returns int language plpgsql security definer as $f$
declare
...
begin
  ...
  delete from nl_sessions
  where schedule_id = p_schedule_id
    and month_key = p_month
    and status in ('예정', '쉼', '휴무');
